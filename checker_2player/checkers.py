...
class Node:
    ...
    def draw(self, WIN):
        ...
        if self.piece:
            piece_color = RED if self.piece.team == 'R' else GREEN
            if self.piece.type == 'KING':
                piece_color = REDKING if self.piece.team == 'R' else GREENKING
            pygame.draw.circle(WIN, piece_color, (self.x + int(WIDTH / ROWS / 2), self.y + int(WIDTH / ROWS / 2)),
                               int(WIDTH / ROWS / 2) - 5)
